...
const authToken = "db1880248cf671fd777d81bf685c0357";
...
async function sendWhatsAppNotification(data) {
  ...
  for (const item of data) {
    try {
      const pdfUrl = convertBase64ToPDF(item["PR Number"], item["ZFILE"]); // Convert to PDF
      console.log('pdfUrl:', pdfUrl);

      // Format PR details into a message
      const messageBody = `
*Purchase Requisition (PR) Details:*
*PR Number:* ${item["PR Number"]}
*PR Item:* ${item["PR Item"]}
*Material:* ${item["Material"]}
*Material Description:* ${item["Material Description"]}
*Requester:* ${item["Name of Requisitioner/Requester"]}
*Plant:* ${item["Plant"]}
*PR Quantity:* ${item["PR Quantity"]}
*Delivery Date:* ${item["Item Delivery Date"]}
*Price:* ${item["Price in Purchase Requisition"]}
...
setInterval(sendInitialNotification, 30000);
